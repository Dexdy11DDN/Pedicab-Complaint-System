...
export const addFranchises = (franchises) => {
  ...
  franchises.forEach(franchise => {
    ...
    const offensesJson = JSON.stringify(franchise.offenses || []);

    db.run(
      `INSERT OR REPLACE INTO franchises 
       (franchiseNumber, ownerName, contactNumber, address, vehicleCount, licenseNumber, status, photos, offenseCount, hasThreeStrikes, offenses, lastSynced)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [
...
export const searchFranchises = (query = '') => {
  ...
  while (stmt.step()) {
    ...
    row.offenses = JSON.parse(row.offenses || '[]');
    row.hasThreeStrikes = !!row.hasThreeStrikes;
    results.push(row);
  }
  stmt.free();
...
export const getFranchiseByNumber = (franchiseNumber) => {
  ...
  if (stmt.step()) {
    ...
    franchise.offenses = JSON.parse(franchise.offenses || '[]');
    franchise.hasThreeStrikes = !!franchise.hasThreeStrikes;
  }
  stmt.free();
